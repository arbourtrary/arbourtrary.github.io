	...
	{#if tree}
		<div class="tree-silhouette">
			<img 
				...
				alt={`Flat single color cut silhouette of the ${tree.h2} tree`}
			/>
			<h2 style={`color: ${tree.color};`}>{@html tree.h2}</h2>
			<h3 style={`color: ${tree.color};`}>{@html tree.h3}</h3>
		</div>

		<div class="button-container" on:click={() => setRandomEntryWithoutReplacement()} on:keydown={() => setRandomEntryWithoutReplacement()}>
			<button > 
